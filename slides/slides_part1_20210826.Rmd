...
params: 
  registered: 75
  goog_sheet: "https://docs.google.com/spreadsheets/d/15f66TJr2ILAsukf7Jfbul-hCc3E-RypCTUD0BnxnmiY/edit?resourcekey#gid=1334483039"
  goog_post:  ""
  days_label: !r c("Fri", "Sat", "Sun", "Mon", "Tue", "Wed")
  days_of_year_number_range: !r seq(232, 237, 1)
output:
...
---
background-image: url(images/who_we_are2_20210826_1.svg)
...
---
background-image: url(images/who_we_are_facet_20210826_1.svg)
background-size: contain
<!-- generated from analysis_workshop_attendance/libcal_roster.Rmd -->
<!-- https://github.com/libjohn/workshop_attendance/blob/master/libcal_roster.Rmd -->
...
data_mgt_plot <- pre_workshop %>% 
  mutate(data_mgt = if_else(data_mgt_satisfy == "Nor sure", "Not Sure", data_mgt_satisfy)) %>% 
  ggplot(aes(fct_relevel(data_mgt_satisfy, "Very unsatisfied", "Unsatisfied", "Neutral", "Satisfied"))) +
  geom_bar() +
  geom_bar(data = . %>% filter(data_mgt_satisfy == orange_bar), aes(data_mgt_satisfy), 
...
In **RStudio**, using _Projects_ (.proj) is a way to manage and organize discrete coding projects. 

- Each folder/directory can be a project
- Works well with _Version Control_
- Promotes **relative file paths**
    - Avoids `setwd()`
- Promotes **reproducibility** by _Restart R and Run all chunks_
    - Avoids `rm(list=ls())` -- not as effective as you want it to be
]
