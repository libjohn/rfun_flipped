...
# gs4_auth_configure()
pre_workshop <- 
  read_sheet("https://docs.google.com/spreadsheets/d/15f66TJr2ILAsukf7Jfbul-hCc3E-RypCTUD0BnxnmiY/edit?resourcekey#gid=1334483039")
# post_workshop <- 
...
tot_registered <- 71
response_rate <- nrow(pre_workshop) / tot_registered
...
pre_workshop %>% 
  ...
  scale_x_continuous(minor_breaks = NULL, 
                     breaks = seq(232, 236, 1),
                     labels = c("Fri", "Sat", "Sun", "Mon", "Tue")) +
  scale_y_continuous(minor_breaks = NULL, breaks = scales::pretty_breaks()) +
  labs(title = "Response rate over time",
       subtitle = glue::glue("n = {tot_registered} ; {scales::percent(response_rate)} response rate"),
       y = "",
...
orange_bar <- fct_count(pre_workshop$data_mgt, sort = TRUE) %>% 
  slice_head(n = 1) %>% 
  pull(f)
...
data_mgt_plot <- pre_workshop %>% 
  ...
  geom_bar() +
  geom_bar(data = . %>% filter(data_mgt == orange_bar), aes(data_mgt), 
...
```{r dta mgt theme_alt, fig.cap="foo", fig.height=7, fig.width=10, message=FALSE, warning=FALSE, dev="svg", echo=FALSE}
data_mgt_plot +
  theme_ft_rc(grid = "Y") +
  theme(plot.title.position = "plot",
        panel.grid.major.x = element_blank())
